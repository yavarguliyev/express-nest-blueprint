...
@CrudTable({ category: 'Database Management', name: 'notifications', displayName: 'Notifications' })
@Injectable()
export class NotificationsRepository extends BaseRepository<NotificationEntity> {
  ...
  async markAllAsRead (recipientId: number): Promise<number> {
    const unreadNotifications = await this.findAll({
      where: { recipientId, isRead: false }
    });

    await Promise.all(
      unreadNotifications.map((notification: NotificationEntity) =>

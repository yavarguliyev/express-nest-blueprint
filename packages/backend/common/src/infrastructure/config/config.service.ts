...
export class ConfigService {
  ...
  getAllKeys = (): string[] => Object.keys(this.envVariables);
  has = (key: string): boolean => key in this.envVariables;
  ...
  private loadEnvironmentVariables(): void {
    ...
    if (!ignoreEnvFile) {
      ...
      if (existsSync(envPath)) {
        ...
        if (result.error) void this.logger.error('Failed to load environment file', result.error.message);
      }
    }

...
terminate_project_processes() {
  ...
  if [ -f "${ROOT_DIR}/packages/infrastructure/docker-compose.prod.yml" ]; then
    print_info "Taking down Docker Compose (prod)..."
    docker-compose -f "${ROOT_DIR}/packages/infrastructure/docker-compose.prod.yml" down --remove-orphans 2>/dev/null || true
  fi

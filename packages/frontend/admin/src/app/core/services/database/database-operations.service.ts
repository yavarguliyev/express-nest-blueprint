...
export class DatabaseOperationsService {
  ...
  updateRecord (table: TableMetadata, id: number, data: Record<string, unknown>): Observable<ApiResponse<Record<string, unknown>>> {
  ...
  bulkDelete (table: TableMetadata, ids: number[]): Observable<ApiResponse<{ deletedCount: number; message: string }>> {
  ...
  confirmDelete (recordId: number, onConfirm: () => void): void {
    this.toastService.confirm(`Mark record ${recordId} for deletion? You can review and apply all changes with "Save Changes".`, onConfirm);
  }

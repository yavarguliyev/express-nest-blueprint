...
import { DateFormatService, TextTransformService, TableStyleService } from '@app/common';
...
export class DatabaseFormattingService {
  ...
  formatValue (value: unknown, column: Column): string {
    ...
    if (column.type === 'datetime') return this.dateFormat.formatForTable(value as string);
  ...
  formatFieldValue (value: unknown): string {
    ...
    if (typeof value === 'string') {
      if (value.includes('admin')) return this.roleAccess.getRoleDisplayName(value);
      return value;
    }
  ...
  getUserInitials (row: Record<string, unknown>): string {
    ...
    else if (firstName) return firstName.charAt(0).toUpperCase();
    else if (lastName) return lastName.charAt(0).toUpperCase();
  ...
  getBooleanValue (
    ...
    if (draftData && columnName in draftData) return draftData[columnName] as boolean;

import { Directive, ElementRef, HostListener, inject, Renderer2, OnInit, OnDestroy, Input, Output, EventEmitter } from '@angular/core';

import { DraggablePosition } from '../../domain/interfaces/draggable-position.interface';
...
export class DraggableResizableDirective implements OnInit, OnDestroy {
  ...
  ngOnInit (): void {
    if (this.enabled) this.setupInteraction();
    if (this.initialPosition) this.applyPosition(this.initialPosition);
  }
  ...
  private deactivate (): void {
    ...
    if (currentPos === 'relative' && !this.initialPosition) this.renderer.removeStyle(element, 'position');
